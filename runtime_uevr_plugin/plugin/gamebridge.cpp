...
bool RuntimeUevrPlugin::init_rendering_pipeline() {
    ...
    return acquire_back_buffers();
}

// Fetches the swapchain's current back buffers and (re)creates their RTVs. Safe to call again
// after on_device_reset() has released the previous references, once the swapchain has actually
// been resized - back_buffers/rtv_heap must already be sized for num_back_buffers.
bool RuntimeUevrPlugin::acquire_back_buffers() {
    const auto uevr_renderer_data = uevr::API::get()->param()->renderer;
    auto device = static_cast<ID3D12Device*>(uevr_renderer_data->device);
    auto swapchain = static_cast<IDXGISwapChain3*>(uevr_renderer_data->swapchain);
...
void RuntimeUevrPlugin::on_present() {
    ...
    // The swapchain was actually resized between the last frame and this one (on_device_reset()
    // already released the old back buffers before that happened) - re-fetch the new ones now
    // that they exist.
    if (needs_reacquire) {
        if (!acquire_back_buffers()) {
            uevr::API::get()->log_error("[XRGB UEVR plugin] Failed to reacquire back buffers after device reset.");
            return;
        }
...
void RuntimeUevrPlugin::on_device_reset() {
    ...
    // IDXGISwapChain::ResizeBuffers - which UEVR calls right after this callback returns - fails
    // (and takes the game down with it) unless every outstanding reference to the current back
    // buffers has been released first. Wait for the GPU to actually be done with each one before
    // releasing our cached ComPtrs, since they may still be in flight from a recent on_present().
    for (int32_t i = 0; i < num_back_buffers; i++) {
        if (fences[i]->GetCompletedValue() < fence_values[i]) {
            WaitForSingleObject(fence_events[i], INFINITE);
            ResetEvent(fence_events[i]);
        }
        back_buffers[i].Reset();
    }

    // The new buffers don't exist yet at this point - the actual resize happens right after this
    // callback returns. on_present() re-fetches them on its next call.
