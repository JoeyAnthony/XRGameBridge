...
XrResult D3D11Renderer::CreateIntermediateTexture(const D3D11ProxySwapchain* back_buffer_swapchain) {
    ...
    info.width = back_buffer_swapchain->GetWidth();
    info.height = back_buffer_swapchain->GetHeight();
...
XrResult D3D11Renderer::CreateWeaver(const D3D11ProxySwapchain* back_buffer_swapchain, const std::shared_ptr<SRSystem> &gb_system) {
...
XrResult D3D11Renderer::CreateWindowSwapchain(const D3D11ProxySwapchain* back_buffer_swapchain, const std::shared_ptr<SRSystem> &gb_system) {
    ...
    create_info.format = back_buffer_swapchain->GetFormat();
...
void D3D11Renderer::InitializePipeline(XrSwapchain swapchain) {
    spdlog::info("Initializing DX11 rendering pipeline");
    auto proxy_swapchain = reinterpret_cast<D3D11ProxySwapchain*>(swapchain);
    std::shared_ptr<SRSystem> system = std::dynamic_pointer_cast<SRSystem>(g_systems[xr_system]);

    CreateCompositor();
    CreateCommandLists();
    CreateIntermediateTexture(proxy_swapchain);

    CreateSystemWindow(system);
    CreateWeaver(proxy_swapchain, system);
    CreateWindowSwapchain(proxy_swapchain, system);
}
