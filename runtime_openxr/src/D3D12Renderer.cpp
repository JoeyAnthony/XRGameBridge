...
namespace XRGameBridge {
    ...
    XrResult D3D12Renderer::RenderFrame(const XrFrameEndInfo* frameEndInfo) {
        ...
        // Render weaving
        if (should_weave) {
            RenderFrameWeaving(frameEndInfo, cmd_list.Get(), window_swapchain, window_swapchain_index, GB_D3D12ProxySwapchain::clear_color, fence_value);
        }
        else {
            RenderFrameSideBySide(frameEndInfo, cmd_list.Get(), window_swapchain, window_swapchain_index, GB_D3D12ProxySwapchain::clear_color, fence_value);
        }
